...
public:
	...
	Colonia(int iteraciones,vector<vector<Num>> & v);
	Colonia(int iteraciones,int hormigas_cant,vector<vector<Num>> & v);
	
	Colonia(int iteraciones,vector<vector<Num>> & matrix,vector<vector<Num>> & matrix2,std::vector<int> Cola,std::vector<int> & npe, std::vector<int> & Vmips);
...
Colonia::Colonia(int iteraciones,vector<vector<Num>> & v){
	vector<Num> temp;
...
Colonia::Colonia(int iteraciones, int hormigas_cant, vector<vector<Num>> & v){
	vector<Num> temp;
	for (int i = 0; i < v.size(); ++i)
	{
		temp.push_back(0.0);
...
Colonia::Colonia(int iteraciones,vector<vector<Num>> & matrix,vector<vector<Num>> & matrix2,std::vector<int> Cola,std::vector<int> & npe, std::vector<int> & Vmips){
	vector<Num> temp;
	cantidad_de_iteraciones=iteraciones;
...
vector<int> Colonia::run(){
	Hormiga MEJOR;

	//mostrar_matriz(matrixdis);
